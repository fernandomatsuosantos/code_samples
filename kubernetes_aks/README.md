...
helm upgrade --install nginx-ingress ingress-nginx/ingress-nginx --namespace infrastructure -f internal-ingress.yml --set controller.replicaCount=2
...
# run httpd at AKS
kubectl apply -f deployment.yml --wait=true
kubectl apply -f service.yml --wait=true
kubectl apply -f ingress.yml --wait=true
```
